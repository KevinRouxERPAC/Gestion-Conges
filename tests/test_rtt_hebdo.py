...
class TestMajAllocationsHebdo:
    def test_recalcul_avec_absence(self, db_session, users, parametrage, allocations):
        parametrage.rtt_calc_mode = "hebdo"
        db.session.commit()
    ...
    def test_mode_non_hebdo_ignore(self, db_session, users, parametrage, allocations):
        # En mode 'fixe', le recalcul hebdomadaire ne fait rien.
        parametrage.rtt_calc_mode = "fixe"
        db.session.commit()
        db.session.add(HeuresHebdo(user_id=users["salarie"].id, date_lundi=date(2026, 6, 1), heures_travaillees=39))
        db.session.commit()
